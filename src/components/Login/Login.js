...
export default class Login extends Component {
  ...
  onSuccess = credentials => {
    auth0.auth
      ...
      .then(profile => this.props.onAuth(credentials, profile))
    ...
    return (
      <KeyboardAvoidingView behavior="padding" style={styles.container}>
        ...
        <View style={styles.tabContainer}>
          <Button onPress={() => this.setState({ viewLogin: true })} title="Log In" />
          <Button onPress={() => this.setState({ viewLogin: false })} title="Sign up" />
        </View>
